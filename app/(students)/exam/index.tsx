    ...
    <View className="flex-1 p-4 bg-white">
      ...
      <View className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6">
        <View className="flex-row items-center mb-2">
          ...
          <Text className="text-amber-800 font-bold text-lg ml-2">Important Notice</Text>
        </View>
      ...
      <Text className="text-2xl font-bold mb-5 text-center">{examInfo.title}</Text>
      
      <View className="bg-gray-100 p-4 rounded-lg mb-5">
        <View className="flex-row justify-between mb-3">
          <Text className="text-base font-semibold">Duration:</Text>
          <Text className="text-base">{examInfo.duration}</Text>
        </View>
        
        <View className="flex-row justify-between mb-3">
          <Text className="text-base font-semibold">Total Marks:</Text>
          <Text className="text-base">{examInfo.totalMarks}</Text>
        </View>

        <Text className="text-base font-semibold mt-3 mb-2">Sections:</Text>
        {examInfo.sections.map((section, index) => (
          <View key={index} className="flex-row justify-between ml-3 mb-1.5">
            <Text>{section.name}</Text>
            <Text>{section.marks} marks</Text>
          </View>
        ))}

        <Text className="mt-4 italic">{examInfo.instructions}</Text>
      </View>

      <Link href={`/exam/${examInfo.id}`} asChild>
        <Pressable className="mt-5 bg-blue-500 rounded-lg py-3 px-4">
          <Text className="text-white text-center font-semibold">Start Exam</Text>
        </Pressable>
      </Link>
    </View>
  );
